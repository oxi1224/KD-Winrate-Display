window.onload = (async () => {
  ...
  const url = 'https://raw.githubusercontent.com/oxi1224/files/main/data-english.json';
  ...
  (async () => {
    ...
    if (/https:\/\/key-drop.com\/(pl|en)\/skins\/category\/.+/.test(window.location.href)) {
      ...
      profitElm.innerHTML = `Chance for X times return:<br>${(Object.entries(caseData.profitChances).map(([k, v]) => `${k}: ${v}`)).join('<br>')}`;
      looseElm.style.cssText = styles.chancesBase + styles.chancesBad;
      looseElm.innerHTML = `Chance for X times loss:<br>${(Object.entries(caseData.looseChances).map(([k, v]) => `${k}: ${v}`)).join('<br>')}`;
      statsElm.style.cssText = styles.chancesBase + styles.winrate;
      statsElm.innerHTML = `<b>Win</b> chance: ${caseData.winChance}%<br><b>Loose</b> chance: ${caseData.looseChance}%`;
      ...
      cases.forEach(c => {
        ...
        if (!goldenNames.includes(caseName[1])) {
          c.addEventListener('mouseover', () => {
            ...
            looseElm.id = 'temp';
            statsElm.style.cssText = styles.chancesBase + styles.winrate;
            statsElm.innerHTML = `${caseData.websiteName}:<br><b>Win</b> chance: ${caseData.winChance}%<br><b>Loose</b> chance: ${caseData.looseChance}%`;
            statsElm.id = 'temp';
            [profitElm, looseElm, statsElm].forEach(e => document.body.appendChild(e));
          });
